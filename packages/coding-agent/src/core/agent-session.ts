...
export class AgentSession {
	...
	private _isRetryableError(message: AssistantMessage): boolean {
		...
		// Match: overloaded_error, provider returned error, rate limit, rate increased (OpenRouter), 429, 500, 502, 503, 504, service unavailable, network/connection errors, fetch failed, terminated, retry delay exceeded
		return /overloaded|provider.?returned.?error|rate.?(limit|increased)|too many requests|429|500|502|503|504|service.?unavailable|server.?error|internal.?error|network.?error|connection.?error|connection.?refused|other side closed|fetch failed|upstream.?connect|reset before headers|socket hang up|timed? out|timeout|terminated|retry delay/i.test(
			err,
		);
	}
